...
void set_ldo4_on()
{
	unsigned int reg;
	int ret;
	ret = regmap_read(Mypca9450->regmap, PCA9450_REG_LDO4CTRL, &reg);
	if (ret) {
		printk( "John_gao Read ldo4 error\n");
		return ;
	}
	printk("John_gao ldo4 : 0x%x \n", reg);
	...
	//set to 2.0 0xcc
	//set to 1.6 0xc8
	regmap_write(Mypca9450->regmap, PCA9450_REG_LDO4CTRL, 0xdf);
	ret = regmap_read(Mypca9450->regmap, PCA9450_REG_LDO4CTRL, &reg);
	if (ret) {
		printk("John_gao Read ldo4 error\n");
		return;
	}
	printk("John_gao set 3.3 ldo4 : 0x%x \n", reg);
}
#endif
